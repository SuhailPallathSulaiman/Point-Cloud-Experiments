...
boost::shared_ptr<pcl::visualization::PCLVisualizer> viewportsVis (pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud1,pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud2,pcl::PointCloud<pcl::PointXYZRGB>::Ptr final)
{
  ...
  PointCloudColorHandlerCustom<pcl::PointXYZRGB> fin_h (cloud2, 0, 0, 255);
...
main (int argc, char** argv)
{
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZRGB> ("Cup/cup9.pcd", *cloud1) == -1)
  {
    PCL_ERROR ("Couldn't read  \n");
    return (-1);
  }
  std::cout << "Loaded 1" ;
    if (pcl::io::loadPCDFile<pcl::PointXYZRGB> ("Cup/cup10.pcd", *cloud2) == -1)
  {
    PCL_ERROR ("Couldn't read  \n");
    return (-1);
  }
